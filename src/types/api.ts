...
export interface GoldenRatioMetric { key: string; label: string; measured: number; ideal: number; deviation_percent: number; score: number; numerator: string; denominator: string; description: string }
export interface GoldenRatioResponse { harmony_score: number; harmony_level: string; metrics: GoldenRatioMetric[]; landmarks: Landmark[]; face_box: Box; quality: QualityReport; recommendations: string[]; processing_ms: number }
