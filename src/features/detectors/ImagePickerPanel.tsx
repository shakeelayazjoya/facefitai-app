...
export function ImagePickerPanel({ onPick, kind = 'face', disabled, processing, loadingLabel = 'Analyzing your photo', title = strings.faceAnalysisScan, description = strings.uploadHelper, uploadAction = strings.uploadSelfie, cameraAction = strings.cameraImage, filePrefix = 'face-scan' }: Props) {
  ...
  return <AppCard style={styles.card}>{processing || busy ? <AnalysisLoader label={processing ? loadingLabel : 'Preparing photo'} /> : <><FeatureArtwork kind={kind} /><AppText variant="h3" align="center" editorial>{title}</AppText><AppText muted align="center">{description}</AppText><View style={styles.tips}><Ionicons name="sunny-outline" color={theme.gold} size={14} /><AppText variant="small" muted>Even light. No filters.</AppText></View><View style={styles.actions}><AppButton disabled={disabled || busy} icon="images-outline" title={uploadAction} onPress={() => void library()} /><AppButton disabled={disabled || busy} icon="camera-outline" title={cameraAction} variant="secondary" onPress={() => void camera()} /></View></>}</AppCard>;
}
const styles = StyleSheet.create({ card: { padding: spacing.sm, alignItems: 'center', gap: spacing.xs }, tips: { width: '100%', flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: spacing.sm, marginTop: spacing.xs }, actions: { width: '100%', gap: spacing.sm, marginTop: spacing.sm } });
