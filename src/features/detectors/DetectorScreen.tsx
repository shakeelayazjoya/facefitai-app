...
export function DetectorScreen({ kind }: { kind: DetectorKind }) {
  ...
  const reset = () => { mutation.reset(); setImageReady(false); setAsset(null); scrollRef.current?.scrollTo({ y: 0, animated: true }); };
  return (
    <ScreenWrapper>
      <ScrollView
        ref={scrollRef}
      ...
      >
        <View style={[styles.content, { maxWidth: responsive.contentWidth }]}> 
          ...
          {!mutation.isPending ? <ResultCards kind={kind} asset={asset} result={mutation.data ?? null} emptyTitle={config.emptyTitle} onReset={mutation.data ? reset : undefined} scanId={kind === 'face' && mutation.data && 'scan_id' in mutation.data ? mutation.data.scan_id : undefined} /> : null}
        </View>
      </ScrollView>
    </ScreenWrapper>
  );
