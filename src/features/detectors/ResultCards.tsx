...
export function ResultCards({ result, emptyTitle, kind, onReset, scanId, asset }: { result: DetectorResult | null; emptyTitle: string; kind: DetectorKind; onReset?: () => void; scanId?: string; asset?: ImageAsset | null }) {
  ...
  const quality = result.quality ?? { quality_score: 90, head_pose_score: 90, blur_score: 90, single_face: true, front_facing: true, lighting: 'good' as const, warnings: [] };
  ...
  const smileEmoji = isExpression(result) ? (result.smile.score >= 30 ? '😄' : result.smile.score >= 12 ? '🙂' : '😐') : null;
    ...
    {isExpression(result) ? (
      <AnimatedCard delay={35}>
        <View style={styles.sectionTitle}>
          <Ionicons name="happy-outline" color={theme.primary} size={21} />
          <AppText variant="h3">Smile detection</AppText>
        </View>
        <View style={styles.smileCardRow}>
          ...
          <View style={{ flex: 1, gap: 4 }}>
            ...
            <AppText muted>Smile intensity score: {Math.round(result.smile.score)}%</AppText>
          </View>
        </View>
      </AnimatedCard>
    ) : null}
