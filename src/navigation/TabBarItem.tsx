...
export const TabBarItem = memo(function TabBarItem({ active, label, icon, activeIcon, width, onPress, onLongPress }: Props) {
  ...
  useEffect(() => { progress.value = withSpring(active ? 1 : 0, { damping: 18, stiffness: 250 }); if (active) bounce.value = withSequence(withTiming(-2, { duration: 80 }), withSpring(0, { damping: 12, stiffness: 250 })); }, [active, bounce, progress]);
  const contentStyle = useAnimatedStyle(() => ({ transform: [{ translateY: bounce.value }, { scale: pressed.value * (1 + progress.value * 0.07) }] }));
  const inactiveStyle = useAnimatedStyle(() => ({ opacity: 1 - progress.value }));
  const activeStyle = useAnimatedStyle(() => ({ opacity: progress.value }));
  const labelStyle = useAnimatedStyle(() => ({ opacity: progress.value, transform: [{ translateY: interpolate(progress.value, [0, 1], [2, 0]) }], maxHeight: interpolate(progress.value, [0, 1], [0, 10]) }));
...
const styles = StyleSheet.create({ pressable: { height: 48, alignItems: 'center', justifyContent: 'center', borderRadius: radii.pill, overflow: 'hidden' }, content: { alignItems: 'center', justifyContent: 'center' }, iconStack: { width: 22, height: 21, alignItems: 'center', justifyContent: 'center' }, icon: { alignItems: 'center', justifyContent: 'center' }, absolute: { position: 'absolute' }, label: { fontSize: 8, lineHeight: 10, fontWeight: '900', letterSpacing: -0.1, textAlign: 'center' } });
